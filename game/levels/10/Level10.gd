...
func _ready():
	globalvar.nowlevel = 10
	_spawn_orbiting($Moon, 3, 85.0, 1.4)
	_spawn_orbiting($Saturn, 4, 140.0, 1.1)
	_spawn_orbiting($Neptune, 4, 155.0, 1.0)
	_spawn_orbiting($SpaceStation, 5, 120.0, 1.5)
...
func _on_RayTimer_timeout():
	add_laser()
	$RayTimer.wait_time = randf_range(1.5, 3.5) * globalvar.get_spawn_interval_mult()

func _on_AsteriodTimer_timeout():
	add_asteriod()
	$AsteriodTimer.wait_time = randf_range(1, 2.5) * globalvar.get_spawn_interval_mult()
