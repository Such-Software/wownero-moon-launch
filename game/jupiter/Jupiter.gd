...
var orbit_speed: float = 0.006  # slowest orbit — Jupiter is massive and far out
var orbit_angle: float = 0.0
var _gravity_radius: float = 0.0
var _body_radius: float = 0.0
const ATMO_COLOR := Color(0.85, 0.6, 0.25)  # amber gas giant
...
func _draw() -> void:
	if _body_radius > 0.0:
		for i in range(5):
			var r := _body_radius + 3.0 + float(i) * 5.0
			var a := lerpf(0.12, 0.02, float(i) / 4.0)
