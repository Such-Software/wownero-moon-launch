...
var orbit_speed: float = 0.003  # barely moves — Pluto is incredibly distant
var orbit_angle: float = 0.0
var _gravity_radius: float = 0.0
var _body_radius: float = 0.0
const ATMO_COLOR := Color(0.7, 0.75, 0.9)  # pale frost
...
func _draw() -> void:
	if _body_radius > 0.0:
		for i in range(3):
			var r := _body_radius + 2.0 + float(i) * 3.0
			var a := lerpf(0.08, 0.02, float(i) / 2.0)
			draw_arc(Vector2.ZERO, r, 0, TAU, 64, Color(ATMO_COLOR.r, ATMO_COLOR.g, ATMO_COLOR.b, a), 1.5)
